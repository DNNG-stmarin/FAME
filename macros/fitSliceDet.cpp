...
void fitSliceDet(){
  ...
  //get psd energy
  TString first = "totPSP>0 & totChan==";
  TString name = "name";
  TString canName;
  TString numDet;
  TString selectChan;
  ...
  for (int j=0; j<NUM_DETS; j++) {
    ...
    numDet = to_string(DETECTORS[j]);
    selectChan = first+numDet;
    canName = name+numDet;

    cout << "DETECTOR: " << DETECTORS[j] << endl;

    TCanvas* canvas = new TCanvas(canName, canName, 800, 500);
    coincTree->Draw("totPSP:totDep>>psdErg", selectChan, "COLZ");
    canvas->Update();
    ...
    for(int i=0; i<bins; i+=step_size) {
      ...
      if(psdErg->ProjectionY("psdErgSlice", i, i+step_size)->GetEntries() > 1000) { //min num entries (500)
        ...
        fitPSD_p->Draw("SAME");
        fitPSD_n->SetLineColor(kPink+10);
        fitPSD_n->Draw("SAME");
