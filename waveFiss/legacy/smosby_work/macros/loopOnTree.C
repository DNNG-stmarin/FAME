...
TChain* makeBigPulseChain()
{
  TChain *pt = new TChain("pt");
  char buf[1000];
  // data 5
  //for (Int_t i=0;i<1085;++i)
  for (Int_t i=0;i<970;++i)
    {
      if (fileGood(i))
        {
          sprintf(buf,"./data/data5_%.3i_pt.root",i);
          pt->Add(buf);
        }
    }
  ...
  /*
  // data 7
  for (Int_t i=0;i<2983;++i)
  {
  if (fileGood(i))
  {
  sprintf(buf,"./data/data7_%3i_pt.root",i);
  pt->Add(buf);
  }
  }

  // data 9
  for (Int_t i=0;i<2639;++i)
  {
  if (fileGood(i))
  {
  sprintf(buf,"./data/data9_%3i_pt.root",i);
  pt->Add(buf);
  }
  }

  // data 12
  for (Int_t i=0;i<3986;++i)
  {
  if (fileGood(i))
  {
  sprintf(buf,"./data/data12_%3i_pt.root",i);
  pt->Add(buf);
  }
  }
  */
