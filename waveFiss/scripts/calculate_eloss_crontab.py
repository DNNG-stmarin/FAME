...
def process_singles(ipath,opath,exepath,runlolim):
    for fname in os.listdir(ipath):
        if fname.endswith('.root'):
            ...
            ofname = 'run%i_eloss.root'%(fnumber)
            ifname2 = 'run%i_evt.root'%(fnumber)
            if (os.path.isfile(opath+ofname) or fnumber < runlolim):
                continue
            else:
                print 'processing file',fnumber
                cmdstr = 'source /home/smosby/.bashrc && %s %s/%s %s/%s %s/%s'%(exepath,ipath,ifname2,ipath,fname,opath,ofname)
...
if __name__=='__main__':
    ...
    exepath = '/home/smosby/tke_analysis/elosscorrections'
